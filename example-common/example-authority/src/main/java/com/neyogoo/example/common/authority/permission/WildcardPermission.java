...
public class WildcardPermission implements Serializable {
    ...
    protected void init(String wildcardString, boolean caseSensitive) {
        if (wildcardString == null || wildcardString.trim().length() == 0) {
            throw new IllegalArgumentException("权限编码通配符字符串不能为 null 或空。确保权限字符串的格式正确。");
        }

        wildcardString = wildcardString.trim();
        List<String> parts = Arrays.stream(wildcardString.split(PART_DIVIDER_TOKEN)).collect(Collectors.toList());

        this.parts = new ArrayList<>();
        for (String part : parts) {
            Set<String> subParts = Arrays.stream(part.split(SUBPART_DIVIDER_TOKEN)).collect(Collectors.toSet());
            if (!caseSensitive) {
    ...
    private Set<String> lowercase(Set<String> subParts) {
        Set<String> lowerCasedSubParts = new LinkedHashSet<>(subParts.size());
