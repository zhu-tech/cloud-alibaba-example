 ...
 * the License.
 */
package com.neyogoo.example.common.database.mybatis.conditions.query;
...
import static com.neyogoo.example.common.database.mybatis.conditions.Wraps.replace;
...
public class LbQueryWrap<T> extends AbstractLambdaWrapper<T, LbQueryWrap<T>>
    ...
    @SafeVarargs
    @Override
    public final LbQueryWrap<T> select(SFunction<T, ?>... columns) {
    ...
    protected LbQueryWrap<T> instance() {
        return new LbQueryWrap<>(
                getEntity(), getEntityClass(), null, paramNameSeq, paramNameValuePairs,
                new MergeSegments(), SharedString.emptyString(),
     ...
     */
    public <A extends Object> LbQueryWrap<T> ignore(BiFunction<T, A, ?> setColumn) {
